import 'package:app_farmacia/bloc/paginas/cadastro_remedios.dart';
import 'package:app_farmacia/bloc/paginas/carrinho_pagina.dart';
import 'package:flutter/material.dart';
import 'package:path/path.dart';

import '../../database/database_helper.dart';
import '../../models/remedio.dart';
import '../paginas/cadastro_remedios.dart';

class RemediosPagina extends StatefulWidget {
  final String categoria;

  const RemediosPagina({
    super.key,
    required this.categoria,
  });

  @override
  State<RemediosPagina> createState() =>
      _RemediosPaginaState();
}

class _RemediosPaginaState
    extends State<RemediosPagina> {
  ...
  Future<void> carregarRemedios() async {
    final lista =
        await DatabaseHelper.instance
            .listarRemedios(widget.categoria);
...
//DEGRADE DA TELA
              ...
              child: ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "R\$ ${remedio.preco.toStringAsFixed(2)}",
                    ),
                  ],
                ),
                trailing: Row(
                  ...
                  children: [
                    ...
                    Checkbox(
                      value: remediosSelecionados.contains(
                        remedio.id,
                      ),
                      onChanged: (value) {
                        setState(() {
                          if (value == true) {
                            remediosSelecionados.add(
                              remedio.id!,
                            );
                          } else {
                            remediosSelecionados.remove(
                              remedio.id,
                            );
                          }
                        });
  ...
  onPressed: () {
    ...
    final selecionados = remedios.where((r) =>
      remediosSelecionados.contains(r.id)
    ).toList();
  ...
  //ABRIR A TELA DO CARRINHO
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CarrinhoPagina(
          remedios: selecionados,
        ),
      ),
    );
  },
